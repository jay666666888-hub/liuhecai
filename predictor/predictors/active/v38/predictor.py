...
class V38NumberPredictor(BasePredictor):
    """v38 6码预测器"""

    def __init__(self, version: str = "v38"):
        super().__init__(version)
        self.params = {"lookback": 50, "top_k": 6}
    ...
    def predict(self, history, top_k=6):
        ...
        result = []
        nums_per_zodiac = max(1, 6 // 12)  # 6个码只选6个生肖
        ...
        final_prediction = [n for n, s in all_scored[:6]]
        ...
        return PredictionResult(
            prediction=final_prediction[:6],
            scores={n: num_scores.get(n, 0) for n in final_prediction[:6]},
            metadata={"mode": "6ma", "top_k": 6}
        )