#!/usr/bin/env python3
"""
V28 平特一肖 - 回测写入账本（直接写入Dashboard数据源）
"""
import sys, os, json
...
from predictor.predictors.active.v28.predictor import V28Predictor
...
def backtest_v28():
    ...
    version = "v28"
    params = pred.params
    lookback = params.get('lookback', 137)
    ...
    for i in range(lookback, len(hist)):
        ...
        record = {
            ...
            "issue": issue,
            "version": version,
            "model": "v28_walk_forward_optimal",
    ...
    # 更新 version_book/v28/metadata.json
    ...
    metadata = {
        ...
        "play_type": "pingte_yixiao",
        "params": params,
        "notes": f"平特一肖 - Walk-Forward验证{hit_rate*100:.2f}%，参数优化搜索20000+10000次",
        "frozen": False,
        "created_at": "2026-05-26T01:40:00.000000",
        "updated_at": now,
        "status": "active",
        "backtest": {
            "total": total,
