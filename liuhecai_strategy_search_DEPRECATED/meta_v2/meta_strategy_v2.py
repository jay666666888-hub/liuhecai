...
class MetaStrategyV2:
    ...
    def _calc_state_features(self, upto_idx: int) -> Tuple[float, float, float, bool]:
        ...
        # 3. 稳定性（专家预测一致性）
        # REAL calculation: use hit_history rolling std normalized by max possible std
        if len(self.hit_history) >= 10:
            recent_hits = self.hit_history[-10:]
            hit_std = np.std(recent_hits)
            # hit_history values are sums of expert hits (0-7), so max std is ~2.5
            max_possible_std = 2.5
            normalized_std = min(hit_std / max_possible_std, 1.0)
        ...
        return entropy, volatility, stability, regime_shift
