...
bp2 = bp.copy()
for _ in range(10000):
    lb = random.randint(max(5, bp['lb']-3), bp['lb']+3)
    g = random.uniform(max(0, bp['g']-0.15), bp['g']+0.15)
    f = random.uniform(max(0, bp['f']-0.15), bp['f']+0.15)
    s = random.uniform(max(0, bp['s']-0.1), bp['s']+0.1)
    t = random.uniform(max(0, bp['t']-0.15), bp['t']+0.15)
    v = random.uniform(max(0, bp['v']-0.15), bp['v']+0.15)
    r = eval_full(hist, lb, g, f, s, t, v)
    if r > best2:
        best2 = r
