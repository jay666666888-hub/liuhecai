...
for _ in range(5000):
    ...
    g = random.uniform(max(-1, bp['g']-0.15), bp['g']+0.15)
    f = random.uniform(max(-1, bp['f']-0.15), bp['f']+0.15)
    s = random.uniform(max(-0.5, bp['s']-0.1), bp['s']+0.1)
    t = random.uniform(max(-1, bp['t']-0.15), bp['t']+0.15)
    v = random.uniform(max(-1, bp['v']-0.15), bp['v']+0.15)
    r = eval_(hist, lb, g, f, s, t, v)
    if r > best2:
        best2 = r
